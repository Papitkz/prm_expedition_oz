/*
  # Add cms_section_videos table and seed missing sections

  1. New Tables
    - `cms_section_videos` — Uploaded/overridden videos per section (one active per section)
      - `id` (uuid, primary key)
      - `section_id` (uuid, foreign key to cms_sections)
      - `section_key` (text)
      - `video_url` (text, the video URL)
  ...
  2. Security
    - RLS enabled on cms_section_videos
    - Public can read videos
    - Only admins can insert/update/delete

  3. Modified Tables
    - `cms_sections` — Added missing sections for blog pages

  4. Important Notes
    - Videos work the same as images: if an active override exists, it takes priority over default_video_url
    - If no active override exists, the default_video_url from cms_sections is used
    - This allows per-section video editing while retaining defaults as fallbacks
*/

-- ============================================
-- CMS SECTION VIDEOS (overridden videos per section)
-- ============================================
...
-- Only admins can update section videos
...
-- Only admins can delete section videos
...
-- Add missing sections for Sylvia and Millenium detail pages
INSERT INTO cms_sections (section_key, page, label, description, default_image_url, default_video_url)
VALUES
...
-- Add itinerary day images for Sylvia
INSERT INTO cms_sections (section_key, page, label, description, default_image_url, default_video_url)
VALUES
...
-- Add itinerary day images for Millenium
INSERT INTO cms_sections (section_key, page, label, description, default_image_url, default_video_url)
VALUES
...
-- Add route map background images
INSERT INTO cms_sections (section_key, page, label, description, default_image_url, default_video_url)
VALUES
...
-- Add vessel gallery images for Sylvia
INSERT INTO cms_sections (section_key, page, label, description, default_image_url, default_video_url)
VALUES
...
-- Add vessel gallery images for Millenium
INSERT INTO cms_sections (section_key, page, label, description, default_image_url, default_video_url)
VALUES
