...
async function loadTrips() {
  loading.value = true
  const { data, error } = await supabase.from('cms_trips').select('*').order('sort_order')
  if (!error && data) trips.value = data as Trip[]
  loading.value = false
}
...
async function loadFeatures(tripId: string) {
  const { data } = await supabase.from('cms_trip_features').select('*').eq('trip_id', tripId).order('sort_order')
  features.value = (data as Feature[]) || []
}

async function loadItinerary(tripId: string) {
  const { data } = await supabase.from('cms_trip_itinerary').select('*').eq('trip_id', tripId).order('day_number')
...
async function saveTrip() {
  ...
  const { error } = await supabase.from('cms_trips').update(updates).eq('id', id)
  ...
  if (!error) {
    ...
    showMessage('Failed to save trip: ' + error.message, 'error')
  }
  saving.value = false
}
...
async function addFeature() {
  ...
  const { data, error } = await supabase.from('cms_trip_features').insert({
...
async function removeFeature(featureId: string) {
  await supabase.from('cms_trip_features').delete().eq('id', featureId)
  features.value = features.value.filter(f => f.id !== featureId)
}

async function addItineraryDay() {
  if (!selectedTrip.value) return
  const { data, error } = await supabase.from('cms_trip_itinerary').insert({
    trip_id: selectedTrip.value.id,
    ...newDay.value,
  }).select().single()
...
async function removeItineraryDay(dayId: string) {
  await supabase.from('cms_trip_itinerary').delete().eq('id', dayId)
  itinerary.value = itinerary.value.filter(d => d.id !== dayId)
}

async function updateItineraryDay(day: ItineraryDay) {
  const { id, trip_id, ...updates } = day
  await supabase.from('cms_trip_itinerary').update(updates).eq('id', id)
...
async function togglePublished() {
  ...
  await supabase.from('cms_trips').update({ is_published: newVal }).eq('id', selectedTrip.value.id)
