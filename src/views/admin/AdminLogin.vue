...
const { signInWithEmail, signInWithGoogle, signUp, loading, isAdmin, firebaseReady, ensureFirebaseInit } = useAdminAuth()
...
ensureFirebaseInit().then(() => {
  if (!firebaseReady.value) needsConfig.value = true
})
...
async function handleEmailAuth() {
  ...
  try {
    if (isRegistering.value) {
      ...
      if (isAdmin.value) router.push('/admin/dashboard')
      else error.value = 'You do not have admin access.'
    }
  } catch (e: any) {
...
<template>
  <div class="login-page">
    <div class="login-card">
      ...
      <div v-if="needsConfig" class="alert alert-error">
        Firebase is not configured yet. Add your Firebase config in the environment variables (VITE_FIREBASE_API_KEY, etc.) or go to Settings after logging in.
      </div>

      <div v-if="error" class="alert alert-error">{{ error }}</div>
      <div v-if="successMsg" class="alert alert-success">{{ successMsg }}</div>

      <form v-if="!needsConfig" @submit.prevent="handleEmailAuth" class="login-form">
